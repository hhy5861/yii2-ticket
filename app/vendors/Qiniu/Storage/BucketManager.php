...
final class BucketManager
{
    ...
    {
        $path = '/stat/' . (new Functions())->entry($bucket, $key);
    ...
    {
        $path = '/delete/' . (new Functions())->entry($bucket, $key);
    ...
    {
	    $func = new Functions();
        $from = $func->entry($from_bucket, $from_key);
        $to = $func->entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        list($_, $error) = $this->rsPost($path);
        return $error;
    }
    ...
    {
	    $func = new Functions();
        $resource = $func->entry($bucket, $key);
        $encode_mime = $func->base64_urlSafeEncode($mime);
    ...
    {
        $resource = (new Functions())->entry($bucket, $key);
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . (new Functions())->entry($bucket, $key));
        }
        return $data;
    }
